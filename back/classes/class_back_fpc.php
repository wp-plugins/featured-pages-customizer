...
class TC_back_fpc {
  ...
  function __construct () {
    ...
    //refresh the post / CPT / page thumbnail on save. Since v1.4
  ...
  /*
  * @return void
  * updates the fpc-thumb-fld post meta with the relevant thumb id and type
  */
  ...
  /**
   *  Binds JS handlers to make FPC preview reload changes asynchronously.
   */
  function tc_customize_preview_js() {
    ...
    wp_enqueue_script(
      'tc-fpc-preview' ,
      sprintf('%1$s/back/assets/js/fpc-customizer-preview.min.js' , TC_FPC_BASE_URL ),
  ...
  function tc_customize_controls_js_css() {
    ...
    wp_enqueue_style(
      ...
      sprintf('%1$s/back/assets/css/fpc-customizer-control.min.css' , TC_FPC_BASE_URL ),
    ...
    if ( 'customizr' == TC_fpc::$theme_name && version_compare( CUSTOMIZR_VER, '3.2.5', '<' ) || 'customizr' != TC_fpc::$theme_name ){
      //ICHECK
      wp_enqueue_style(
        'fpc-icheck-style',
        sprintf('%1$s/back/assets/css/icheck.min.css' , TC_FPC_BASE_URL ),
        array( 'customize-controls' ),
        TC_fpc::$instance -> plug_version,
        $media = 'all'
      );
      wp_enqueue_script(
        'icheck-script',
        //dev / debug mode mode?
        sprintf('%1$s/back/assets/js/lib/lib_icheck.js' , TC_FPC_BASE_URL ),
        $deps = array('jquery'),
        TC_fpc::$instance -> plug_version,
        $in_footer = true
      );
      ...
      wp_enqueue_style(
        'fpc-selecter-style',
        sprintf('%1$s/back/assets/css/selecter.min.css' , TC_FPC_BASE_URL ),
        array( 'customize-controls' ),
        TC_fpc::$instance -> plug_version,
        $media = 'all'
      );
      wp_enqueue_script(
        'selecter-script',
        //dev / debug mode mode?
        sprintf('%1$s/back/assets/js/lib/lib_selecter.js' , TC_FPC_BASE_URL ),
        $deps = array('jquery'),
        TC_fpc::$instance -> plug_version,
        $in_footer = true
      );
    ...
    wp_enqueue_script(
      'tc-fpc-controls' ,
      sprintf('%1$s/back/assets/js/fpc-customizer-control.js' , TC_FPC_BASE_URL ),
      array( 'customize-controls' ),
      TC_fpc::$instance -> plug_version ,
      true
    );
  ...
  /*
    ...
    */
    function tc_print_js_templates() {
      ?>
      <script type="text/template" id="fpc_cta">
        <div class="tc-cta fpc-in-control-cta-wrap">
        <hr/>
          <?php
            printf('<span class="tc-notice">%1$s</span><a class="tc-cta-btn" href="%2$s" title="%3$s" target="_blank">%3$s &raquo;</a>',
              __( "Need more pages and features ?" , TC_fpc::$instance -> plug_lang ),
              sprintf('http://%sextension/featured-pages-unlimited/', 'presscustomizr.com/' ),
              __( "Get Featured Pages Unlimited" , TC_fpc::$instance -> plug_lang )
            );
          ?>
        </div>
      </script>
